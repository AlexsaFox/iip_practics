...
def output_counts_of_model_by_algo(ms: ModelStorage):
    ...
    for key, value in m.items():
        print(f'по алгоритму {key} найдено {value} моделей')
...
def draw_histogram(ms: ModelStorage, fieldname, ax):
    ...
    ax.set_title(f'Гистограмма распределения {fieldname}')
...
def draw_scatter(ms: ModelStorage, fieldname1, fieldname2, ax):
    ...
    ax.set_title(f'{fieldname1} и {fieldname2} - диаграмма рассеяния')
    ax.set_xlabel(fieldname1)
    ax.set_ylabel(fieldname2)
...
def draw_boxplot(ms: ModelStorage, fieldname1, fieldname2, ax):
    ...
    ax.set_title(f'{fieldname1} и {fieldname2}')
    ax.set_xlabel(fieldname1)
    ax.set_ylabel(fieldname2)
    ax.set(xlim=(min(data_1) - 1, max(data_1) + 1),
           ylim=(min_y - 1, max_y + 10))
